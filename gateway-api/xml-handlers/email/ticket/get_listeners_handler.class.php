 ...
 *
 */
class get_listeners_handler extends xml_parser
{
   /**
    ...
    */
   function get_listeners_handler(&$xml) {
      $this->xml =& $xml;
   }
   ...
   function process() {
      ...
      if(is_object($tickets_obj)) {
         $children = $tickets_obj->get_children();
         if(is_array($children['ticket'])) {
            foreach($children['ticket'] as $key=>$ticket_obj) {
               $tickets[] = $ticket_obj->get_attribute('id', FALSE);
            }
         }
      }

      if($views_obj->get_listeners($tickets) === FALSE) {
         xml_output::error(0, 'Get ticket listeners failed');
      }
      else {
         xml_output::success();
      }
