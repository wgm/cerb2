 ...
 *
 */
class assign_address_handler extends xml_parser
{
   /**
    ...
    *
    * @param object $xml
    * @return assign_address_handler
    */
   function assign_address_handler(&$xml) {
      $this->xml =& $xml;
   }
   ...
   function process() {
      ...
      $address = $this->xml->get_child_data("address", 0);
      
      $obj = new general_contacts();
      
      if($obj->assign_address($contact_id, $address) === FALSE) {
         xml_output::error(0, 'Failed to assign address for contact');
      }
      else {
         xml_output::success();
      }
