...
/*!
	...
	\brief PEAR Mime Mail E-mail Class.

	\note PEAR Core classes adapted by WebGroup Media LLC
	
	\author Richard Heyes, richard@phpguru.org
	\version 1.4
	\date (c) Copyright 2001-2002
*/
...
*/
	...
	$cfg = CerConfiguration::getInstance();
        ...
        */
		...
		$this->build_params['text_encoding'] = '8bit'; // [BGH]: set to 8bit
		$this->build_params['html_charset']  = LANG_CHARSET_CODE;
		$this->build_params['text_charset']  = LANG_CHARSET_CODE;
		$this->build_params['head_charset']  = LANG_CHARSET_CODE;
        ...
        */
		...
		$this->smtp_params['host'] = $cfg->settings["smtp_server"]; // 'localhost'
		$this->smtp_params['port'] = 25;
		$this->smtp_params['helo'] = $helo;
		$this->smtp_params['auth'] = (defined('SMTP_AUTH_USERNAME') && SMTP_AUTH_USERNAME != '') ? TRUE : FALSE;
		$this->smtp_params['user'] = defined('SMTP_AUTH_USERNAME') ? SMTP_AUTH_USERNAME : '';
		$this->smtp_params['pass'] = defined('SMTP_AUTH_PASSWORD') ? SMTP_AUTH_PASSWORD : '';
        ...
        */
		...
		/**
        * [JAS]: Ensure we have our default date header in RFC822 compliant format
        */
		$this->setHeader('Date', date('D, d M Y H:i:s O'));

 		/**
 		* [JXD]: Set Precedence to bulk to avoid responses from vacation program etc.
 		*/
 		if($cfg->settings["send_precedence_bulk"]) $this->headers['Precedence'] = 'bulk';
...
*/
	function setTextEncoding($encoding = '8bit') // [BGH]: changed default to 8bit
	{
...
*/
	function setTextCharset($charset = LANG_CHARSET_CODE)
	{
...
*/
	function setHtmlCharset($charset = LANG_CHARSET_CODE)
	{
...
*/
	function setHeadCharset($charset = LANG_CHARSET_CODE)
	{
...
*/
	...
	{
		$text = str_replace("\r\n","\n",$text); // [JAS]: make sure everything is equal.
		$text = str_replace("\n","\r\n",$text); // [JAS]: replace the above fix, along with bare LF
		$this->text = $text;
	}
...
*/
	...
	{
		$params['content_type'] = LANG_CHARSET_MAIL_CONTENT_TYPE;
		$params['encoding']     = $this->build_params['text_encoding'];
		$params['charset']      = $this->build_params['text_charset'];
		
		if(LANG_CHARSET_MAIL_CONTENT_TYPE == "text/html")
			$text = nl2br($text);
...
*/
	function buildMessage($params = array())
	{
		...
		if (isset($message)) {
			...
			// [BGH] Only add if we haven't already specified it.
...
*/
	function _encodeHeader($input, $charset = LANG_CHARSET_CODE)
	{
	...
	function send($recipients, $type = 'mail')
	{
		...
		switch ($type) {
			case 'mail':
				...
				$this->return_path = ""; // [JAS]: PHP 4.2.x+ Temporary FIX
...
*/
	function getRFC822($recipients)
	{
		...
		if (isset($this->reply_to)) {
			$headers[] = 'Return-To: ' . $this->reply_to;
		}
