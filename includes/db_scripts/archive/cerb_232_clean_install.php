...
|
| Purpose: Installs a clean 2.3.2 database structure
|
...
define("DB_SCRIPT_NAME","Cerberus Helpdesk 2.3.2 Release Clean Database Install");
define("DB_SCRIPT_AUTHOR","WebGroup Media LLC.");
define("DB_SCRIPT_DATE","20040126");
...
function set_precursor_hashes()
{
	...
	if($TABLE_PM->table_exists) {
		...
		$cerberus_db->query($sql);
		$sql = "REPLACE INTO db_script_hash(script_md5,run_date) VALUES('4287a900f4c322c2590fc8a99836d10a',NOW())"; // 2.3.1 to 2.3.2
		$cerberus_db->query($sql);
	}
}
...
function init_table_configuration()
{
	...
	$TABLE_DEF->create_sql = "CREATE TABLE `configuration` (".
		...
		"`user_only_assign_own_queues` TINYINT DEFAULT '0' NOT NULL,".
		...
		"`parser_version` CHAR(32) DEFAULT '' NOT NULL,".
		"`save_message_xml` TINYINT DEFAULT '1' NOT NULL,".
		"PRIMARY KEY  (`cfg_id`)".
		") TYPE=MyISAM;"; 
...
function init_table_knowledgebase_problem()
{
	...
	$TABLE_DEF->create_sql = "CREATE TABLE `knowledgebase_problem` (".
  		...
  		"`kb_problem_text_is_html` TINYINT DEFAULT '0' NOT NULL,".
...
function init_table_knowledgebase_solution()
{
	...
	$TABLE_DEF->create_sql = "CREATE TABLE `knowledgebase_solution` (".
  		...
  		"`kb_solution_text_is_html` TINYINT DEFAULT '0' NOT NULL,".
