...
define("CER_PLUGIN_NAME","vBulletin 3.x Login Handler");
define("CER_PLUGIN_TYPE","login");
define("CER_PLUGIN_AUTHOR","WebGroup Media LLC.");
define("CER_PLUGIN_CLASS","plugin_loginVBulletin3"); // [JAS]: The function to act as main()

class plugin_loginVBulletin3 extends cer_LoginPlugin {
	var $remote_db = null;
	var $login_string = "Forum Login";
	var $password_string = "Forum Password";
	
	function plugin_loginVBulletin3($plugin_id,&$params) {
	...
	function getRemoteUserId() {
		...
		$sql = sprintf("SELECT u.userid ".
					   "FROM user u ".
					   "WHERE u.username = %s AND u.password = MD5(%s) ",
